#!/usr/bin/env python3
"""
Applicazione per Gestione Spedizioni BRT - PyQt5 (Windows Version)
...
class BRTSpedizioniApp(QMainWindow):
    ...
    def update_navigation_buttons(self):
        ...
        self.prev_btn.setEnabled(bool(self.current_index > 0))

        # Bottone Salta: disabilita se siamo all'ultimo
        self.skip_btn.setEnabled(bool(not is_last))

        # Bottone "Vai a Saltati": mostra SOLO se ci sono record saltati
        # Converti numpy.int64 in int Python per il confronto
        if int(skipped_records) > 0:
        ...
        if bool(all_completed):
            ...
            self.save_next_btn.setEnabled(bool(can_save))
        ...
        self.export_btn.setEnabled(bool(all_completed))
